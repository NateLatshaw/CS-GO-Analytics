...
LR3 <- glm(T_win ~ t_eq_val + ct_eq_val + seconds + round_type + RoundState + BombLocation, 
           data = train_df, family = 'binomial')
summary(LR3)
LR4 <- glm(T_win ~ t_eq_val + ct_eq_val + T_HP_remaining + CT_HP_remaining + seconds + round_type + RoundState + BombLocation, 
           data = train_df, family = 'binomial')
summary(LR4)
...
# random forests
# outcome = T wins round
RF1 <- ranger(factor(T_win) ~ RoundState + bomb_site + round_type + map + t_eq_val + ct_eq_val + seconds, 
...
x_train <- model.matrix(T_win ~ Tval_minus_CTval + seconds + round_type + RoundState + 
                          Tval_minus_CTval * RoundState + BombLocation, 
                        data = train_df)
y_train <- as.matrix(train_df[, T_win])
x_test <- model.matrix(T_win ~ Tval_minus_CTval + seconds + round_type + RoundState + 
                         Tval_minus_CTval * RoundState + BombLocation, 
                       data = test_df)
y_test <- as.matrix(test_df[, T_win])

cv_lasso <- cv.glmnet(x_train, y_train, alpha = 1, family = 'binomial', type.measure = 'mse')
lasso <- glmnet(x_train, y_train, alpha = 1, family = 'binomial')
test_df[, probslasso := predict(lasso, newx = x_test, s = cv_lasso$lambda.min, type = 'response')]
test_df <- CalibratedProbs(df_ = test_df, model_ = 'lasso', interval_width_ = .04)
rm(cv_lasso, lasso)
gc()

cv_ridge <- cv.glmnet(x_train, y_train, alpha = 0, family = 'binomial', type.measure = 'mse')
ridge <- glmnet(x_train, y_train, alpha = 0, family = 'binomial')
test_df[, probsridge := predict(lasso, newx = x_test, s = cv_lasso$lambda.min, type = 'response')]
test_df <- CalibratedProbs(df_ = test_df, model_ = 'ridge', interval_width_ = .04)
rm(cv_ridge, ridge)
gc()
